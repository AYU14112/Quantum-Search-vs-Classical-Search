# main.py
"""
Main controller for Search Benchmark Suite.
Improved UI, pretty results, optional graphing per single-run and full benchmark.
Run: python main.py
"""
import sys
import random
...
from linear_search import linear_search_repeated, pretty_print_linear_result, plot_linear_result
from binary_search import binary_search_with_optional_sort, pretty_print_binary_result, plot_binary_search_steps
from quantum_search import run_grover_repeated
...
def run_single_algorithm_ui():
    ...
    if choice == 1:
        ...
        repeats = choose_int("Repeats (median)", default=3, minv=1)
        res = linear_search_repeated(ds, target, repeats=repeats)
        pretty_print_linear_result(res)
        if want_plot:
            plot_linear_result(res, size)
        ...
        res = binary_search_with_optional_sort(ds, target, do_sort=do_sort)
...
def run_scalability_compare_ui():
    ...
    repeats = choose_int("Repeats per point (median)", default=2, minv=1)
    ...
    for n in range(1, max_qubits + 1):
        ...
        # classical (use unsorted dataset to be fair)
        ds_unsorted = list(range(N))
        random.shuffle(ds_unsorted)
        lin = linear_search_repeated(ds_unsorted, target, repeats=repeats)
        classical_steps.append(lin['steps'])
        classical_times.append(lin['elapsed_py_impl'])

        # binary
        ds_for_bin = ds_unsorted.copy()
        b = __import__('binary_search').binary_search_with_optional_sort(ds_for_bin, target, do_sort=include_sort)
        ...
        try:
            ...
            quantum_iters.append(np.nan)
            quantum_times.append(np.nan)
            quantum_success.append(np.nan)
            print(f"Quantum run failed at n={n}: {e}")

        print(f"n={n} (N={N}) -> linear steps={lin['steps']}, binary_total_ms={b['elapsed_total']*1000:.2f}, quantum_iters={quantum_iters[-1]}")

    # Plot steps (log-log)
    plt.figure(figsize=(10,6))
    plt.plot(Ns, classical_steps, 'o-', label='Classical steps (measured)')
    # quantum_iters may have nan; handle safely
    ...
    plt.xlabel('Dataset size N (log scale)'); plt.ylabel('Steps / Iterations (log scale)')
    ...
    plt.xscale('log'); plt.yscale('log')
    plt.xlabel('Dataset size N (log scale)'); plt.ylabel('Time (ms, log scale)')
